...
def get_video_info(video_path, min_pixels, max_pixels, width, height, fps, nframes=None, data_root=None):
    # Using this because of process_vision_info function
    # Need to fix this in the future
    if nframes:
        content = {
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources:
            ...
            video_input, video_kwargs = get_video_info(
                ...
                width=self.video_resized_w,
                height=self.video_resized_h,
                fps=self.fps, nframes=self.nframes,
...
class SafeSupervisedDataset(SupervisedDataset):
    def __init__(self, *args, max_retry=10, **kwargs):
    ...
    def __getitem__(self, index):
        ...
        while retry_count < self.max_retry:
            try:
                ...
                retry_count += 1
                print(f"[WARN] Failed to load index {index}: {e}, retrying {retry_count}/{self.max_retry}...")
                index = random.randint(0, len(self) - 1)

        raise RuntimeError(
            f"[FATAL] SafeSupervisedDataset: Failed to load a valid sample after {self.max_retry} retries. "
            f"Initial index was {orig_index}."
        )
...
def make_supervised_data_module(model_id, processor, data_args):
    ...
    # )
    sft_dataset = SafeSupervisedDataset(
        data_path=data_args.data_path, processor=processor, data_args=data_args, model_id=model_id
    )
