...
from training.trainer import PPETrainer
...
def train():
    ...
    os.environ["USE_LIGER"] = str(use_liger)
    if use_liger:
    ...
    if data_args.ppe_config and os.path.exists(data_args.ppe_config):
        with open(data_args.ppe_config, "r") as f:
    ...
    # save the original rope_scaling.mrope_section (from pretrained model, [16, 24, 24] (default), or [32, 32])
    original_mrope_section = model.config.rope_scaling.get("mrope_section")
    ...
    # recover the original rope_scaling.mrope_section（[16, 24, 24] (default), or [32, 32])
    # we save the origin setting in config.json to keep better generalized usage, the section would only be changed 
    # when applying PPE when doing forward and it would not influence the config.json file
    model.config.rope_scaling["mrope_section"] = original_mrope_section
